# Security & Implementation Guidelines (CLAUDE.md)

This project must follow these 12 strict security rules and AI-specific guidelines for all modifications, additions, and deployments.

---

## 1. Secrets and Environment Variables
* **Never expose secrets in frontend code.** Every API key, token, database URL, and private config lives in `.env` files only.
* `.env` files must always be listed in `.gitignore` (`.env`, `.env.local`, `.env.*.local`).
...
* If a secret must be used client-side (e.g. Stripe publishable key), add a comment explaining it is a public key intentionally exposed.
...
* **Validate and sanitize everything on the server.** Client-side validation is a UX convenience, not a security measure.
...
Before every deploy, verify this checklist:
